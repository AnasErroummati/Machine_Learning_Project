...
# This flag is used to define the mode of execution as an argument
...
#This flag is used to take the data directory as input
...
# this flag is used to get the checkpoint 
flags.DEFINE_string(
    'checkpoint', None,
...
# this function oad data and applies changes to it if necessary.
...
# if max size flag is specified do the necessary changes

    if max_size is not None:
        dataset = dataset.take(max_size)
# This method combines multiple consecutive elements of this dataset, which might have different shapes, into a single element. 
    ...
    dataset = dataset.prefetch(tf.data.experimental.AUTOTUNE) # prefetch transformation, which can be used to decouple the time when data is produced from the time when data is consumed. ... In particular, the transformation uses a background thread and an internal buffer to prefetch elements from the input dataset ahead of the time they are requested.
...
# loading the checkpoint file + load hyperparameters
